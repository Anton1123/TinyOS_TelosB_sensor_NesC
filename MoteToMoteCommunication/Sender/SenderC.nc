...
{	
	...
	event void LightRead.readDone(error_t result, uint16_t val)
	{
		if(result == SUCCESS)
		{
			...
			{
				if(front == NULL) //if empty queue, we proceed with sending current packet
				{
					uint8_t i;
					//Creating the Packet
					...
					call Timer.stop();
					msg -> msgID = packetID++;
					msg -> tag = 0; //will check for priority soon
					for(i = 0; i < 12; i++)
					{
						msg -> lumArray[i] = tempArray[i];
					}
					
					//Enqueue the sent packet if its a priority packet
					if(1)
					{
						msg -> tag = 1;
						Enqueue(*msg);
				...
				else //queue not empty, first send the priority packet, pop the queue, and proceed with current readings
				{
					//Creating the Packet
					MoteToMoteMsg_t* msg = call Packet.getPayload(& _packet, sizeof(MoteToMoteMsg_t)); 
					call Timer.stop();
					*msg = Front(); //problem here
